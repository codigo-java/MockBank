...
public class ContaBusiness {
	...
	public void transferir(TransferenciaDTO transferenciaDto) {
		Conta contaOrigem = contaRepository.findByAgenciaAndNumero(transferenciaDto.getAgencia(),
				transferenciaDto.getNumContaOrigem());
		if (contaOrigem == null) {
			LOG.error("MB-013: Conta {} não localizada para agência {} ", transferenciaDto.getNumContaOrigem(),
					transferenciaDto.getAgencia());
		...
		Conta contaDestino = contaRepository.findByAgenciaAndNumero(transferenciaDto.getAgencia(),
		...
		DepositoDTO depositoDto = new DepositoDTO(contaDestino.getAgencia(), contaDestino.getNumero(),
	...
	public void depositar(DepositoDTO depositoDto) {
		...
		if (conta == null) {
			LOG.error("MB-013: Conta {} não localizada para agência {} ", depositoDto.getNumeroConta(),
	...
	public void sacar(SaqueDTO saqueDto) {
		...
		if (conta == null) {
			LOG.error("MB-013: Conta {} não localizada para agência {} ", saqueDto.getNumeroConta(),
					saqueDto.getAgencia());
	...
	private void validaContaCorrente(Conta conta) {
		List<Pessoa> titularesPJ = conta.getTitulares().stream()
				.filter(p -> p.getTipoPessoa().equals(TipoPessoa.JURIDICA)).collect(Collectors.toList());
		List<Pessoa> titularesPF = conta.getTitulares().stream()
				.filter(p -> p.getTipoPessoa().equals(TipoPessoa.FISICA)).collect(Collectors.toList());
	...
	public List<Conta> findAll(FindContaDTO findContaDTO) {
		Specification<Conta> spec = Specification.where(null);

		if (findContaDTO.getNumberAccount() != null) {
			spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("numero"),
					findContaDTO.getNumberAccount()));
		}
		if (findContaDTO.getBankBranch() != null) {
			spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("agencia"),
					findContaDTO.getBankBranch()));
		}
		if (findContaDTO.getAccountType() != null) {
			TipoConta tipo = TipoConta.getTipoConta(findContaDTO.getAccountType());
			spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("tipoConta"), tipo));
		}

		return contaRepository.findAll(spec);
	}
